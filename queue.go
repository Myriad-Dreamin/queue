...
// put queue functions
func (q *EsQueue) Put(val uint32) (ok bool, quantity uint32) {
...
func (q *EsQueue) Puts(values []uint32) (puts, quantity uint32) {
...
// round 到最近的2的倍数, 注意v > 2^{63}时有问题
func minQuantity64(v uint64) uint64 {
	v--
	v |= v >> 1
	v |= v >> 2
	v |= v >> 4
